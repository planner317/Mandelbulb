...
uniform float moffX, moffY, moffZ, mmix;
uniform float incX, incY, incZ;
uniform float scaleX, scaleY, scaleZ;
uniform bool isFullRender;
uniform bool useShadow;
uniform bool useAO;
uniform bool toggleMandelBulb;
uniform bool toggleMengerSponge;
...
#else
        if(angle.x > 0.0)
            z.yz = rotate(angle.x, z.yz);
        if(angle.y > 0.0)
            z.zx = rotate(angle.y, z.zx);
        if(angle.z > 0.0)
            z.xy = rotate(angle.z, z.xy);
#endif        
        z += jVec;
...
vec3 Phong(vec3 light, vec3 eye, vec3 pt, vec3 N, vec3 mat) {
    ...
    vec3 lambertian = mat * (lightIntensity + abs(N) * normalComponent) * max(NdotL, 0.0) * diffIntensity;
...
vec2 mainBulb(vec3 p) {
    ...
    for(int i = int(maxIterations); i > 0; i--) {
        ...
        vec3 q = mix(vec3(moffX, moffY, moffZ), p, mmix);
        v.xyz = vec3(sin(theta) * cos(phi), sin(phi) * sin(theta), cos(theta)) * zr + q;
        trap = min(trap, dot(v.xyz, v.xyz));
    }
    float r = length(v.xyz);
    return vec2(0.5 * log(r) * r / v.w, trap + 2.0);
